...
\begin{document}
\newgeometry{left=5mm,right=5mm,top=2.5cm,bottom=5cm}
\begin{figure}[h]
\centering
\begin{tabular}{l|l|l|l@{\;\,\vline\,}c@{\,\vline\,}c@{\,\vline\;\,}l}
  ...
  \hline
  \ttfamily arnew &         & i64:$n$         & R$^*$ && & create array of reals (init: 0) of length $n\geq0$ \\
  \ttfamily arld  &         & R$^*$ i64:$k$   & R     &&y& load element $k$ of array of reals to stack \\
  \ttfamily arst  &         & R$^*$ i64:$k$ R &       &&y& store to element $k$ of array of reals \\
  \hline
  \ttfamily aznew &         & i64:$n$         & Z$^*$ && & create array of integers (init: 0) of length $n\geq0$ \\
  \ttfamily azld  &         & Z$^*$ i64:$k$   & Z     &&y& load element $k$ of array of integers to stack \\
  \ttfamily azst  &         & Z$^*$ i64:$k$ Z &       &&y& store to element $k$ of array of integers \\
  \hline
  ...
  \hline
  \ttfamily rlim  & adr:$f$ & $a_1..a_n$ & $b_0..b_m$ &?&y& $\lim_{p\to\infty}{(f(-p,a_1,\ldots,a_n))_p} = (b_0,\ldots,b_m)$, $n,m$ unspec. \\[-1.35ex]\hline\noalign{\vspace{\dimexpr 1.35ex-.5pt}}
  \ttfamily rch   &         & R:$r_1..r_n$ i64:$n$ & i64:$k$    &y&& mv-choice, $\{0\}$ if all $<0$, $\{i:r_i>0\}$ otherwise \\
  \ttfamily rapx  &         & R:$x$ i64:$p$   & Z:$m$ i64:$e$   &y&& approx.\ to absolute prec.: $|x-m\cdot2^{e-\lceil\log_2(|m|+1)\rceil}|<2^p$ \\
  ...
  \ttfamily lvc   & u64:$n$ & Z:$\tilde p$ $a_1'..a_n'$ & $a_1..a_n$   &&y& leave continuous section ($\tau(a_i)\in\{\text{R,K}\}$ only if not last)
\end{tabular}
