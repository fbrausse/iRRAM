...
public:
	...
	friend RATIONAL_SEP operator+(const RATIONAL_SEP & x,
	                              const RATIONAL_SEP & y);
	friend RATIONAL_SEP operator-(const RATIONAL_SEP & x,
	                              const RATIONAL_SEP & y);
	friend RATIONAL_SEP operator*(const RATIONAL_SEP & x,
	                              const RATIONAL_SEP & y);
...
RATIONAL_SEP::RATIONAL_SEP(const REAL & r_arg, sizetype sep1_arg,
                           sizetype sep2_arg, int len_arg)
{
	r = r_arg;
	sep1 = sep1_arg;
	sep2 = sep2_arg;
...
void compute()
{
	...
	for (int i = 0; i <= count; i++) {
		if ((i < 100) || (i % 10) == 0) {
			cout << x.r << " " << i << "\n"
			     << " separators: " << x.sep1.mantissa << "*2^"
			     << x.sep1.exponent
			     << " separators: " << x.sep2.mantissa << "*2^"
			     << x.sep2.exponent << "\n"
