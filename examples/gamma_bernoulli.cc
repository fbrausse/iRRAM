...
RATIONAL bernoulli(const unsigned int index)
{
	...
	while (bernoulli_min_unknown <= index) {
		...
		if (bernoulli_min_unknown % 2 == 0) {
			RATIONAL summe = bernoulli_saved[0] +
			                 binomial_saved[1] * RATIONAL(-1, 2);
			for (unsigned int i = 2; i < bernoulli_min_unknown;
			     i += 2) {
				summe = summe + bernoulli_saved[i / 2] *
				                        binomial_saved[i];
			}
			summe = -1 * summe /
			        binomial_saved[bernoulli_min_unknown];
...
REAL ln_gamma_approx(int max_index, const REAL & z)
{
	...
	for (int index = 1; index <= max_index; index++) {
		coefficient = REAL(bernoulli(2 * index)) / (2 * index - 1) /
		              (2 * index) * inv_power_z;
		result += coefficient;
...
REAL gamma(const REAL & x) { return limit(gamma_approx, x); }
