...
void compute()
{
	...
	for (long fkt = min_fkt; fkt <= max_fkt; fkt++) {
		...
		if (fkt == 8)
			value[1] = approx(tan(x), prec);
		if (fkt == 9)
			value[1] = approx(atan(x), prec);
		if (fkt == 10)
			value[1] = approx(tanh(x), prec);
		if (fkt == 11)
			value[1] = approx(atanh(w), prec);
		if (fkt == 12)
			value[1] = approx(euler(), prec);
		if (fkt == 13)
			value[1] = approx(pi(), prec);
